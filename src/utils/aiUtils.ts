...
export const getAIResponse = async (conversation: any, userMessage: string, extraMessages: any[] = []): Promise<string> => {
  try {
    ...
    try {
      ...
      return "I'm having trouble connecting to my services right now. This might be a temporary connection issue. Could you share your thoughts again in a moment?" +  JSON.stringify(error);
    }
  } catch (error: any) {
